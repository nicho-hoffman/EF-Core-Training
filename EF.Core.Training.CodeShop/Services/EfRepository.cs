...
namespace EF.Core.Training
{
    ...
    public class EfRepository : IRepository
    {
        ...
        {
            var books = apiContext.AuthorBookLinks.Where(x => x.BookID == bookID);
            foreach (var bookLink in books)
            {
                apiContext.AuthorBookLinks.Remove(bookLink);
            }
        ...
        {
            var authorIDs = await apiContext.AuthorBookLinks.Where(x => x.BookID == bookID).Select(x => x.AuthorID).Distinct().ToListAsync();
            return await apiContext.Authors.Where(x => authorIDs.Contains(x.ID)).ToListAsync();
        }
