...
namespace EF.Core.Training
{
    ...
    public class EfRepository : IRepository
    {
        ...
        public async Task<bool> DeleteAuthorBookLinksForBook(int bookID)
        {
            try
            {
                var authorLinks = await RetrieveAuthorBookLinksByBookID(bookID);
                bool result = false;

                foreach (AuthorBookLink authorLink in authorLinks)
                    result = await DeleteAuthorBookLink(authorLink) || result;
        ...
        {
            return await apiContext.Authors.Where(x => x.BookLinks.Any(bl => bl.BookID == bookID)).ToListAsync();
        }
        ...
        {
            return await apiContext.Books.Where(x => x.AuthorLinks.Any(a => a.AuthorID == authorID)).ToListAsync();
        }

        public async Task<ICollection<Book>> RetrieveBooksByGenreID(int genreID)
        {
            return await apiContext.Books.Where(x => x.GenreLinks.Any(g => g.GenreID == genreID)).ToListAsync();
        }
